...
CREATE TABLE IF NOT EXISTS employee_contracts (
 ...
 notes TEXT, created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS employee_documents (
 ...
 document_type TEXT NOT NULL, title TEXT NOT NULL, expires_at TEXT, status TEXT NOT NULL DEFAULT 'active', created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS employee_salary_history (
 id BIGSERIAL PRIMARY KEY, tenant_id INTEGER NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
 employee_id INTEGER NOT NULL REFERENCES employees(id) ON DELETE CASCADE, salary REAL NOT NULL, currency TEXT NOT NULL DEFAULT 'EGP', effective_from TEXT NOT NULL, effective_to TEXT, reason TEXT, created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS employee_reporting_history (
 ...
 effective_from TEXT NOT NULL, effective_to TEXT, reason TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS quote_events (
 ...
 changed_by INTEGER REFERENCES users(id), note TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS payment_allocations (
 ...
 amount REAL NOT NULL CHECK(amount >= 0), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(tenant_id,payment_id,invoice_id)
);
CREATE TABLE IF NOT EXISTS refunds (
 ...
 amount REAL NOT NULL CHECK(amount > 0), reason TEXT, status TEXT NOT NULL DEFAULT 'pending', created_by INTEGER REFERENCES users(id), processed_at TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS credit_notes (
 ...
 reason TEXT, status TEXT NOT NULL DEFAULT 'draft', created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(tenant_id,note_number)
);
CREATE TABLE IF NOT EXISTS task_events (
 ...
 user_id INTEGER REFERENCES users(id), payload TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS notification_deliveries (
 ...
 attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT, delivered_at TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS file_access_log (
 ...
 ip_address TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS audit_entity_links (
 ...
 created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(tenant_id,entity_type,entity_id,related_type,related_id)
);
...
CREATE TABLE IF NOT EXISTS backup_runs (
 id BIGSERIAL PRIMARY KEY, tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE, backup_type TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS system_health_checks (
 id BIGSERIAL PRIMARY KEY, tenant_id INTEGER REFERENCES tenants(id) ON DELETE CASCADE, component TEXT NOT NULL,
 status TEXT NOT NULL, latency_ms REAL, details TEXT, checked_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
